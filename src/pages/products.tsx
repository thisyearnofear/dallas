...
const CATEGORIES: {
    id: AgentProtocol["category"] | "all";
    label: string;
    icon: string;
}[] = [
    { id: "all", label: "All Protocols", icon: "🌐" },
    { id: "context_management", label: "Context Windows", icon: "🧠" },
    { id: "tool_calling", label: "Tool Calling", icon: "🛠️" },
    { id: "evaluation", label: "Evaluations", icon: "📊" },
    { id: "orchestration", label: "Orchestration", icon: "🤖" },
];

export function Products() {
    const [selectedCategory, setSelectedCategory] = useState<
        AgentProtocol["category"] | "all"
    >("all");
    ...
    const totalMembers = agentProtocols.reduce(
        (sum, p) => sum + p.memberCount,
        0,
    );
    const totalCaseStudies = agentProtocols.reduce(
        (sum, p) => sum + p.optimizationLogCount,
        0,
    );
        ...
        <div class="min-h-screen transition-colors duration-300">
            ...
            <div class="text-center mb-12">
                ...
                <p class="text-xl mb-8 max-w-3xl mx-auto text-slate-600 dark:text-slate-300">
                    Discover community-tracked agent architecture protocols.
                    Real benchmarks, real data, real agents.
                </p>
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4 max-w-2xl mx-auto mb-8">
                    ...
                    <div class="bg-white dark:bg-slate-900 p-4 rounded-lg border border-brand/20 shadow-sm transition-colors">
                        <div class="text-2xl font-bold text-brand">
                            {totalCaseStudies.toLocaleString()}
                        </div>
                        <div class="text-sm text-slate-600 dark:text-slate-400 font-medium">
                            Optimization Logs
                        </div>
            ...
            <div class="mt-16 text-center">
                <div class="bg-gradient-to-r from-slate-800 to-slate-900 text-white p-10 rounded-2xl shadow-xl border border-slate-700">
                    ...
                    <p class="text-lg mb-8 max-w-2xl mx-auto text-slate-300">
                        Join our community of agent architecture experimenters.
                        Share your journey, track your outcomes, and help others
                        discover what works.
                    </p>
            ...
            <div class="mt-12 p-6 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-xl">
                <p class="text-sm text-yellow-800 dark:text-yellow-300 text-center">
                    <strong>⚠️ Important:</strong> These protocols are
                    community-shared experiences, not guaranteed solutions.
                    Always verify agent architecture benchmarks before
                    integration. Results vary by individual implementation.
                    Agent Alliance does not endorse or guarantee any specific
                    outcomes.
                </p>
            </div>
        </div>
